...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Функция безопасного email для Firebase
  ...
  if (btnRegister) {
    btnRegister.addEventListener('click', async () => {
      ...
      console.log('✅ Код сохранён:', sEmail);
      ...
      if (!sent) {
        if (emailSuccess) emailSuccess.textContent = '⚠️ Код: ' + currentVerificationCode;
      } else {
        if (emailSuccess) emailSuccess.textContent = '✅ Код отправлен';
      }
      
      if (emailModal) {
        emailModal.style.display = 'flex';
        console.log('✅ Окно верификации');
      }
  ...
  if (btnVerifyEmail) {
    btnVerifyEmail.addEventListener('click', async () => {
      ...
      if (!pendingRegistration) {
        if (emailError) emailError.textContent = 'Начните заново';
        return;
      }
  ...
  if (btnResend) {
    btnResend.addEventListener('click', async () => {
      ...
      if (emailSuccess) {
        emailSuccess.textContent = sent ? '✅ Отправлен' : '⚠️ Код: ' + currentVerificationCode;
      }
      if (emailError) emailError.textContent = '';
    });
  }
  ...
  if (loginPassword && btnLogin) {
    loginPassword.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') btnLogin.click();
    });
  }
  if (loginUsername && btnLogin) {
    loginUsername.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') btnLogin.click();
    });
  }
  if (regPasswordConfirm && btnRegister) {
    regPasswordConfirm.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') btnRegister.click();
    });
  }
  if (verificationCodeInput && btnVerifyEmail) {
    verificationCodeInput.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') btnVerifyEmail.click();
    });
  }

  // ===== ЗАПУСК =====
  checkSession();
  console.log('✅ Limessage готов!');
